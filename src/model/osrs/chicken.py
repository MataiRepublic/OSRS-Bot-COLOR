...
class OSRSchicken(OSRSBot):
    ...
    def main_loop(self):
        ...
        while time.time() - start_time < end_time:
            ...
            #toggles run when at 90%
            if self.get_run_energy() >=90:
            ...
            # While not in combat
            while not self.api_morg.get_is_in_combat():
                # Find a target
                target = self.get_nearest_tagged_NPC()
                if target is None:
                    failed_searches += 1
                    self.camera_rotate('right', 30)
            ...
            # While in combat
            while self.api_morg.get_is_in_combat():
                # Check to eat food
                if self.get_hp() < self.hp_threshold:
                    self.__eat(self.api_status)
                time.sleep(1)

            # Loot all highlighted items on the ground
            if self.loot_items:
                self.__loot(self.api_status)
    ...
    def green_tile(self):
        ...
        time.sleep(3)
    def blue_tile(self):
        tile = self.get_all_tagged_in_rect(self.win.game_view, clr.BLUE)
        self.mouse.move_to()
        self.mouse.click()
        time.sleep(3)
    def purple_tile(self):
